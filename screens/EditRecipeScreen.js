...
function EditRecipeScreen({ navigation, route }) {
  const { recipe, categories: cat } = route.params;

  const [loading, setLoading] = useState(false);

  const [hours, minutes] = recipe.minutes
    .replace(' час(а/ов) ', '_')
    .replace('минут', '')
    .trim()
    .split('_');
  ...
  const [photo, setPhoto] = React.useState(recipe.image ? `${baseURL}/${recipe.image}` : null);
  ...
  useEffect(() => {
    (async function () {
      setLoading(true);
      const newCat = cat.map((el) => el.Categorie.id_category);
      ...
      delete nutritionData.id_nutrition;
      delete nutritionData.recipe_id;
      ...
      const editIngredients = ingredientsData.map((el) => ({
        ingredient: el.Ingredient.id_ingredient,
        amount: el.amount,
      }));

      setField((prevState) => [...editIngredients, ...prevState]);
  ...
  const onSubmit = useCallback(async () => {
    ...
    await editMyRecipe(formData, recipe.id_recipe);

    if (photoResult) {
      await publishImageRecipe(recipe.id_recipe, createFormData(photoResult));
    }
  ...
  return (
    <ScrollView style={[globalStyles.screen, { padding: 30 }]}>
      ...
      <View style={{ paddingBottom: 300 }}>
        ...
        >
          <Text style={{ textAlign: 'center', color: '#fff', fontSize: 17 }}>Изменить рецепт</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
