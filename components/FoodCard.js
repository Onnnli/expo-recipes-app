...
export default function FoodCard({ navigation, route, food }) {
  const foodColor = useMemo(() => {
    const result = foodCategory.filter(({ name }) => name === food.type[0])[0];
    return result?.color ? result.color : `#FEA11F`;
  }, [food]);
    ...
    <View style={styles.cardWrapper}>
      ...
      >
        ...
        <View style={styles.cardDetailsContainer}>
          ...
          <Text style={{ color: '#444', fontStyle: 'italic', fontSize: 12, marginTop: -4 }}>
            {food.tagalog}
          </Text>
